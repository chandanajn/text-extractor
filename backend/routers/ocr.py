...
from schemas.ocr import OCRRecord as OCRRecordSchema, PaginatedOCRResponse
...
@router.post("/upload", response_model=OCRRecordSchema, status_code=status.HTTP_201_CREATED)
...
def search_ocr(
    ...
    if q:
        search_filter = or_(
            OCRRecord.filename.ilike(f"%{q}%"),
            OCRRecord.text.ilike(f"%{q}%")
        )
...
def get_ocr_record(id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    record = db.query(OCRRecord).filter(OCRRecord.id == id, OCRRecord.user_id == current_user.id).first()
    if not record:
        raise HTTPException(status_code=404, detail="Record not found")
    return record

@router.get("/{id}/pdf")
def get_ocr_pdf(id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    record = db.query(OCRRecord).filter(OCRRecord.id == id, OCRRecord.user_id == current_user.id).first()
    if not record:
        raise HTTPException(status_code=404, detail="Record not found")
...
def delete_ocr_record(id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    record = db.query(OCRRecord).filter(OCRRecord.id == id, OCRRecord.user_id == current_user.id).first()
    if not record:
        raise HTTPException(status_code=404, detail="Record not found")
...
def clear_ocr_history(db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
