...
def generate_ocr_pdf(record: OCRRecord) -> io.BytesIO:
    ...
    # Title
    Story.append(Paragraph(f"OCR Extraction Report", styles['Title']))
    ...
    Story.append(Paragraph(f"<b>Confidence:</b> {record.confidence:.2f}%", styles['Normal']))
    Story.append(Paragraph(f"<b>Processing Time:</b> {record.processing_time:.2f}s", styles['Normal']))
    Story.append(Paragraph(f"<b>Date:</b> {record.created_at.strftime('%Y-%m-%d %H:%M:%S')}", styles['Normal']))
    Story.append(Spacer(1, 12))
